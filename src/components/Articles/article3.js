...
<p>The flow system was modeled in a discrete event simulation tool, Arena, developed by Rockwell Automation. Discrete event simulation is a method that models the behavior of a system by focusing on state changes that occur at specific moments, allowing the analysis of complex processes over time and the evaluation of the impact of different variables.</p>

<h4>a. Description of Variables</h4>
<p>The simulation duration was set to 221 days for a work period of 11 effective hours per day, representing the high season, followed by 98 days for a work period of 8 hours, representing the low season of inland navigation on the Garonne River and its canal.</p>
<p>We replicated this simulation 10 times to compare each replication and consolidate the results. Replication helps reduce uncertainty because by multiplying replications, the impact of random fluctuations is diminished. This helps to validate the consistency of the results. In this case, the time is being evaluated.</p>
<p>In this study, we evaluate six performance indicators:</p>
<ul>
...
<p>To model uncertainties, we identified two main factors: the tidal range, which represents a significant source of variation, and the waiting times at locks, which can impact the navigation time and the speed of boat rotations. We now aim to evaluate how these two uncertainties (tidal range and waiting times) will affect the system's operation in the long term.</p>

<p>The waiting time at locks is estimated between 15 and 30 minutes, with a uniform probability distribution.</p>
...
<table>
  ...
  <tbody>
    {/* <!-- Example rows, add actual data here --> */}
    <tr>
      <td>2024-04-01</td>
      <td>12:00</td>
      <td>18:00</td>
...
<p>In this analysis method, we aim to measure the impact of uncertainties on the rotation time. This will contribute to evaluating the robustness of the economic study, especially regarding the decision variable V2, which is the frequency of river unit rotations (see page 23).</p>
...
<p>We first evaluate the rotation of the river unit from the port quay, crossing the locks and the Garonne River, the unloading time at the Grand Port Maritime de Bordeaux (GPMB), and then the return via the Garonne and the lateral canal locks.</p>
<p>Two scenarios are proposed depending on whether it is the high season or low season.</p>
<p>The process begins with the successive loading of 8 trucks at the shipper's location. Once the trucks are loaded, they are unloaded, and their contents are transferred to a river unit. After being fully loaded, the unit begins its transit, passing through 13 locks and the Garonne before arriving at GPMB, where it is unloaded. The return of the river unit is empty. Each step of the process, from loading to return, is timed. The distances traveled and transport speeds are also integrated into the model for accurate representation.</p>
...
<table>
  ...
  {/* <!-- Include your analysis table here --> */}
</table>
...
<table>
  ...
  {/* <!-- Include your performance analysis for case #2 --> */}
</table>
